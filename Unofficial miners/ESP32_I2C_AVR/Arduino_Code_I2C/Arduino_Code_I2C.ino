...
void setup() {
  ...
  Wire.begin(I2C_ID);
  Wire.onReceive(receiveEvent); // receive job
  Wire.onRequest(requestEvent); // send job
...
void receiveEvent(int len)
{
  ...
  if(Wire.available()){
    ...
    difficulty = strtoul(tdiff.c_str(), NULL, 10);
